...
class HeartModule(BaseModule):
    ...
    def _load_frame(self, spec: ModuleSpec) -> pd.DataFrame:
        frame = pd.read_csv(spec.dataset_root / "heart_2020_cleaned.csv")
        frame["HeartDisease"] = frame["HeartDisease"].map({"No": 0, "Yes": 1})
        return frame
    ...
    def train(self, spec: ModuleSpec, mode: str, **kwargs) -> dict[str, Any]:
        ...
        train, val, test = split_binary_frame(
            frame,
            spec.target_column or "HeartDisease",
            smoke_rows=spec.smoke_rows if mode == "smoke" else None,
        )

        numeric_columns = ["BMI", "PhysicalHealth", "MentalHealth", "SleepTime"]
        categorical_columns = [column for column in frame.columns if column not in numeric_columns + [spec.target_column]]
        ...
        best_name, best_model, candidate_scores, calibrator = select_best_binary_model(
            ...
            train_frame=train[numeric_columns + categorical_columns],
            train_target=train[spec.target_column],
            val_frame=val[numeric_columns + categorical_columns],
        ...
        val_raw = best_model.predict_proba(val[feature_columns])[:, 1]
        test_raw = best_model.predict_proba(test[feature_columns])[:, 1]
        ...
        joblib.dump(train[feature_columns].sample(min(256, len(train)), random_state=42), artifact_dir / "background_frame.joblib")
